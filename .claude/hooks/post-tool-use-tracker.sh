...
detect_skill_domain() {
    ...
    elif [[ "$file" =~ /runner ]] || [[ "$file" =~ /timeout ]] || [[ "$file" =~ /skill-writer ]] || [[ "$file" =~ /skill-reader ]] || [[ "$file" =~ /prompts/ ]]; then
        detected_skills="claude-runner"
    elif [[ "$file" =~ /cli ]] || [[ "$file" =~ /index ]] || [[ "$file" =~ /errors ]]; then
        detected_skills="cli-shell"
    elif [[ "$file" =~ /target ]] || [[ "$file" =~ /target-transform ]] || [[ "$file" =~ /backend ]] || [[ "$file" =~ /AGENTS ]] || [[ "$file" =~ /.aspens ]]; then
        detected_skills="codex-support"
    elif [[ "$file" =~ /doc-impact ]] || [[ "$file" =~ /impact ]] || [[ "$file" =~ /impact-analyze ]] || [[ "$file" =~ /impact.test ]] || [[ "$file" =~ /doc-impact.test ]]; then
        detected_skills="doc-impact"
    elif [[ "$file" =~ /doc-sync ]] || [[ "$file" =~ /diff-classifier ]] || [[ "$file" =~ /diff-helpers ]] || [[ "$file" =~ /git-hook ]] || [[ "$file" =~ /git-helpers ]] || [[ "$file" =~ /doc-sync-refresh ]] || [[ "$file" =~ /preservation-contract-refresh ]]; then
        detected_skills="doc-sync"
    elif [[ "$file" =~ /graph-builder ]] || [[ "$file" =~ /graph-persistence ]] || [[ "$file" =~ /parsers/ ]] || [[ "$file" =~ /doc-graph ]] || [[ "$file" =~ /graph-context-prompt ]]; then
        detected_skills="import-graph"
    elif [[ "$file" =~ /scanner ]] || [[ "$file" =~ /source-exts ]] || [[ "$file" =~ /path-resolver ]] || [[ "$file" =~ /typescript ]] || [[ "$file" =~ /python ]] || [[ "$file" =~ /nextjs ]] || [[ "$file" =~ /scan ]] || [[ "$file" =~ /scanner.test ]]; then
        detected_skills="repo-scanning"
    elif [[ "$file" =~ /save-tokens ]] || [[ "$file" =~ /hooks/ ]] || [[ "$file" =~ /save-handoff ]] || [[ "$file" =~ /commands/ ]] || [[ "$file" =~ /tests/ ]]; then
        detected_skills="save-tokens"
    elif [[ "$file" =~ /doc-init ]] || [[ "$file" =~ /context-builder ]] || [[ "$file" =~ /prompts/ ]]; then
        detected_skills="skill-generation"
    elif [[ "$file" =~ /add ]] || [[ "$file" =~ /add-skill ]] || [[ "$file" =~ /templates/ ]]; then
