...
You are working in **aspens** — a CLI tool that generates and maintains AI-ready documentation (skill files + CLAUDE.md) for any codebase. Supports multiple output targets (Claude Code, Codex CLI).
...
- `aspens doc init [path]` — Generate skills + hooks + CLAUDE.md (supports `--target claude|codex|all`, `--backend claude|codex`)
