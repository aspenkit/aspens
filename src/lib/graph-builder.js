...
export async function buildRepoGraph(repoPath, languages = []) {
  ...
  for (const absPath of filePaths) {
    ...
    if (JS_EXTS.has(ext)) {
      const parsed = parseJsImports(content, relPath);
...
const SKIP_DIRS = new Set([
  ...
  '.next', 'vendor', 'coverage', 'third_party',
]);
...
function parseJsImports(content, relPath) {
...
function loadPathAliases(repoPath) {
  ...
  for (const configName of ['tsconfig.json', 'jsconfig.json']) {
    const configPath = join(repoPath, configName);
    ...
    try {
      ...
      if (paths) {
        for (const [pattern, targets] of Object.entries(paths)) {
          ...
          const replacement = join(repoPath, baseUrl, target);
